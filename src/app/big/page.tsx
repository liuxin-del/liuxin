...
const Upload = () => {
  ...
  //合并切片
	const maegeRequest = (fileHash:string,fileName:string)=>{
		fetch(`http://localhost:3000/liuXin/merge`,{
			method:"POST",
			headers:{
  ...
  const verify = (fileHash:string,fileName:string) => {
    console.log(fileHash,fileName,'22222222222222');
        return fetch('http://localhost:3000/liuXin/verify',{
          method:'POST',
          headers:{
            'content-type':'application/json'
          },
   ...
   const uploadChunks = async (chunks:Blob[],fileHash:string,fileName:string,existChunks:string[])=>{
      ...
      console.log('当前的fileHash值：', fileHash);
         ...
         while(index<formDatas.length){
            const task = fetch('http://localhost:3000/liuXin/upload',{
               method:"POST",
	...
	const handleUpload = async(e: Event) => {
      ...
      setFileHash(hash as string);
		// 请求服务器，判断是否秒传
    const data=await verify(hash as string,fileName as string);
    console.log(data,'秒传')
